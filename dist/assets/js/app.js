...
const filterTriggers = document.querySelectorAll("[data-filter-by]");
if (filterTriggers) {
  filterTriggers.forEach((trigger) => {
    const type = trigger.dataset.filterBy;
    const filterItems = trigger.querySelectorAll(
      `[data-filter] [data-${type}]`
    );
    const listItems = trigger.querySelectorAll(`[data-items] [data-${type}]`);
    ...
    filterItems.forEach((item) => {
      item.addEventListener("click", () => {
        ...
        listItems.forEach((_item) => {
          if (
            _item.dataset[type].toLowerCase() !==
